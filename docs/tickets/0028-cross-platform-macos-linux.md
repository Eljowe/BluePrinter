...
Green-lit. Recorded in [ADR-0006](../adr/0006-cross-platform-macos-linux.md),
which supersedes ADR-0001.

- **Targets**: macOS universal (arm64 + x86_64), minimum macOS 11; Linux Ubuntu
  22.04+ x86_64. (macOS and Linux both in scope.)
- **Formats**: macOS = AU + VST3 + Standalone; Linux = VST3 + Standalone.
- **macOS signing**: **unsigned for v1**; notarization deferred until a paid
  Apple Developer account exists. The build must not assume notarization.
- **Linux packaging**: AppImage + `.deb`; WebKitGTK **4.1** declared/documented
  as the runtime dependency.
- **WebView backend**: per-platform — `webview2` / `wkwebview` / `webkitgtk`.
- **Crash diagnostics**: a **POSIX handler** (`sigaction` + backtrace) writing the
  same `crash-info.txt` the self-heal parser reads; graceful degradation.
- **CI**: build + test on `windows-latest`, `macos-latest`, `ubuntu-latest`.

## Implementation tickets

- [0042](0042-portable-build-foundation-ci.md) — portable build foundation +
  macOS/Linux CI (unblocked; do this first).
- [0043](0043-portable-crash-diagnostics.md) — portable crash diagnostics
  (after 0042).
- [0044](0044-macos-packaging.md) — macOS packaging, unsigned (after 0042).
- [0045](0045-linux-packaging.md) — Linux AppImage + `.deb` (after 0042).
