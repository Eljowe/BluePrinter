...
- **MIDI clock**: one **header-level** toggle (`midiClockEnabled` in `HeaderControls.jsx`) — when on, the clock **free-runs** (`clockRunning` atomic; if no transport step (count-in/recording/looper) advanced `metronomePosition` in a block, the clock advances it itself), so a drum machine runs without recording. The free-running clock also renders the audible metronome click (`renderMetronomeInBlock`, subject to the metronome toggle). Takes and loop captures **ride the same clock** — actual-recording/capture time fires Start again (`midiStartPending` + `sendDirectMidiStart`) so external gear re-syncs on the take's first beat. Toggling sends Start/Stop **directly to the output device** (`sendDirectMidiStart`/`sendDirectMidiStop`, `midiOutputLock`) — the standalone never forwards the host MIDI buffer to hardware, so without the direct send a drum machine stays silent. The MIDI out device picker also lives in the header. **Click gating**: the master click on/off is `metronomeEnabled` (header-level, shared by count-ins, takes, loop captures and the free-running clock); the header-level `clickDuringCapture` (default on) is shared by the take and the looper — when off the click only plays during the count-in.
- **Audio looper**: The sequencer panel was replaced with a pure audio looper. It captures the record mix (dry + selected chains, so synth sounds and FX are baked into the loop), plays back audio-only, and supports click/count-in, crop start/end in whole beats, loop/one-shot, and saving the loop as a library snippet.
